...
partial class Room : GameWindow{
	...
	public static void Main(string[] args){
		#if WINDOWS
			if(GetConsoleWindow() == IntPtr.Zero || Console.OpenStandardOutput() == Stream.Null){
	...
	private void loadProperty(string line){
		try{
			...
			switch(words[0].TrimEnd()){
				...
				case "@maxFps":
				this.UpdateFrequency = (int) UInt32.Parse(words[1]);
				break;
	...
	private void tryLoadTextures(){
		for(int i = 0; i < textures.Length; i++){
			if(textures[i] != null && File.Exists(textures[i].Substring(1))){
				Texture2D t;
				if(textures[i][0] == 'L'){
					t = Texture2D.fromFile(textures[i].Substring(1), TextureParams.Smooth, i + 1);
				}else if(textures[i][0] == 'N'){
					t = Texture2D.fromFile(textures[i].Substring(1), TextureParams.Default, i + 1);
				}else{
					t = Texture2D.fromFile(textures[i].Substring(1), TextureParams.Default, i + 1);
				}
	...
	protected override void OnLoad(){
		...
		if(uniformBackBuffer){
			...
			this.mainShader.setInt("iBackBuffer", 0);
		}
	...
	protected override void OnRenderFrame(FrameEventArgs args){
		...
		if(uniformHour){
			mainShader.setVector3("iHour", new Vector3(now.Hour, now.Minute, now.Second)); //Set the hour
		}
		
		if(uniformDate){
			mainShader.setVector3("iDate", new Vector3(now.Day, now.Month, now.Year)); //Set the date
		}
